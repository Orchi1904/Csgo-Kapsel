...
export default async function getCapsules(): Promise<CapsuleData[]> {
  ...
  // Todo: Replace with revalidate, after API is working again
  ...
  /*Fetch new data if data is older than 8 hours, because 
      ...
      -> As of May 28th 2024, the data is only fetched from firebase and not from 
         csgobackpack API anymore, because it seems that this API was removed. 
         Todo: Replace Csgobackpack API with Steam-Links in JSON-Format:
...
async function updateCapsules() {
  const response = await fetch(
    "http://csgobackpack.net/api/GetItemsList/v2/?currency=EUR&extend=1"
  );
  ...
  for (const capsule of Capsules.major_capsules) {
    ...
    const svpRatio =
      stickerValue !== "N/A" && average_price !== "N/A"
        ? stickerValue / average_price
        : "N/A";
